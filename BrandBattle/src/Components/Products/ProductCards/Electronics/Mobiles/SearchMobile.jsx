...
import mobilesData from '../../../../Data/mobileData.json';      // your JSON
import MobileCard from './MobileCard';         // import your MobileCard
...
function SearchMobile() {
  ...
  useEffect(() => {
    if (query.trim()) {
      setSuggestions(
        mobilesData.filter(m => m.name.toLowerCase().includes(query.toLowerCase()))
      );
    } else {
      setSuggestions([]);
  ...
  const doSearch = (term) => {
    ...
    const found = mobilesData.find(m => m.name.toLowerCase() === t);
    setSelectedMobile(found || null);
    setSuggestions([]);
  };
  ...
  return (
    <div className="bg-gradient-to-br from-[#d4deff] via-[#bfcfde] to-[#99a8e5] w-full  flex flex-col items-center p-4 px-4">
      ...
      {/* Render the same MobileCard for the selected result */}
      {selectedMobile && (
        <div className="mt-10">
          <MobileCard mobile={selectedMobile} />
        </div>
      )}

      {/* No match message */}
      {!selectedMobile && query && !suggestions.length && (
        <motion.p
          className="mt-10 text-[#192654] font-semibold"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          No mobile found for “{query}”
        </motion.p>
      )}
    </div>
  );
